...
const fragment3 = document.createDocumentFragment() //dar nombre d etiqueta d elemento a crear párrafo(p) capa(div)
...
const pintarCards = data => {
    ...
    data.forEach(item => {
		...
		`
		templateCard3.querySelector('a').innerHTML = `<a href="${item.url_ml}"><button class="personalizado_">Mercado libre</button></a>` 
        templateCard3.querySelector('button').dataset.id = item.id
		//templateCard3.querySelector('h5').textContent = item.title
		templateCard3.querySelector('h5').innerHTML = `<h5 id="${item.title}">${item.title}</h5>`
        templateCard3.querySelector('span').textContent = item.precio
        templateCard3.querySelector('p').textContent = item.descripcion
        templateCard3.querySelector('div').innerHTML = `
		<div id="carousel-example-generic" class="carousel slide" data-ride="carousel" style="height:auto; margin:0;">
			<div class="carousel-inner">
			    <div class="item active">
			    	<img id="myImg" src="${item.im1}" alt="First slide"/>
